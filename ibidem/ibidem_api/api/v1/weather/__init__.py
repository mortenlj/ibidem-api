...
async def http_client():
    headers = {
        "user-agent": f"ibidem-api/{get_version()} +https://github.com/mortenlj/ibidem-api"
    }
...
async def weather() -> WeatherResponse:
    return WeatherResponse(
        icon_name="heavysnowshowersandthunder_polartwilight", temperature=-99.9
    )


@router.get(
    "/icon/{icon_name}", status_code=status.HTTP_200_OK, response_class=FileResponse
)
async def retrieve_icon(
    icon_name, http_client: Annotated[httpx.Client, Depends(http_client)]
) -> Path:
